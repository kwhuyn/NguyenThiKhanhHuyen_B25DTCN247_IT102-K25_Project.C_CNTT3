...
bool is_valid_date(int day, int month, int year) {
    if (year < 1 || year > 2025 || month < 1 || month > 12 || day < 1) {
        return false;
    }
...
int isValidPhone(char phone[]) {
    ...
    if(len <= 9 || len >= 15) {
        printf("=> LOI: So dien thoai phai co do dai tu 10 den 14 so!\n");
        return 0;
    }
...
void addPatient(){
    ...
    do {
        printf("Nhap so dien thoai (10-14 so): ");
...
void updatePatient(){
    ...
    do {
        printf("Nhap so dien thoai moi (10-14 so, Hien tai: %s): ", patients[idx].phone);
        fgets(tempInput, sizeof(tempInput), stdin);
        tempInput[strcspn(tempInput, "\n")] = 0;
        valid = 1;
        if(strlen(tempInput) > 0){
  	...
  	char tempDebtStr[ID_LEN];
    float tempDebt = -1.0;
    printf("Nhap debt moi (>=0, Hien tai: %.f): ", patients[idx].debt);
    fgets(tempDebtStr, sizeof(tempDebtStr), stdin);
    tempDebtStr[strcspn(tempDebtStr, "\n")] = 0;

    if(strlen(tempDebtStr) > 0){
        tempDebt = atof(tempDebtStr);
        if(tempDebt >= 0){
            patients[idx].debt = tempDebt;
        } else {
            printf("=> CANH BAO: Gia tri debt moi khong hop le (>=0). Giu lai gia tri cu.\n");
        }
    }
